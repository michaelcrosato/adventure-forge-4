...
- **Conversation mode** (`"dialogue": true`) folds them behind one `talk to
  ...
  give every conversation-mode npc a farewell. A topic with `"commits": true`
  (an oath broken, a betrayal) is listed late in the conversation, next to a
  line that parts ways, so a menu that shrinks as questions are answered never
  slides it into the number the player has been pressing. Gate later topics on
