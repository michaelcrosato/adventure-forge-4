...
type Shape = {
  rooms: number; bare: number; bareIds: string[];
...
const blank = (): Shape => ({ rooms: 0, bare: 0, bareIds: [], landmarks: 0, actions: 0, variants: 0, sideTrips: 0, npcs: 0, talkers: 0, topics: 0, items: 0, quests: 0, epilogue: 0, fates: new Set() });
...
for (const [id, room] of Object.entries(world.rooms)) {
  ...
  if (!(room.actions ?? []).length && !peopled.has(id) && !stocked.has(id)) { s.bare += 1; s.bareIds.push(id); }
}
...
console.log("region  rooms  corridors      lmk  acts  varis  npcs  talk  topics  items  quests  epil  fates");
...
for (const [r, s] of rows) {
  ...
  const flag = pct >= 15 ? " <" : "";
  console.log(
    `${r.padEnd(6)} ${pad(s.rooms, 6)}  ${pad(`${s.bare} (${pct}%)`, 9)}${flag.padEnd(2)} ${pad(s.landmarks, 4)} ${pad(s.actions, 5)} ${pad(s.variants, 6)} ${pad(s.npcs, 5)} ${pad(s.talkers, 5)} ${pad(s.topics, 7)} ${pad(s.items, 6)} ${pad(s.quests, 7)} ${pad(s.epilogue, 5)}  ${[...s.fates].sort().join("/") || "—"}`,
  );
}
